...
// #include <ygm/mailbox_p2p_nlroute.hpp>
...
class visitor_queue {
  ...
  // typedef ygm::mailbox_p2p_noroute<visitor_type, mailbox_recv> mailbox_type;
  // typedef ygm::mailbox_p2p_nlroute<visitor_type, mailbox_recv> mailbox_type;
 ...
 public:
  ...
  void clear() {
    local_queue_type m_local_own_empty, m_local_delegate_empty;
    std::queue<visitor_type> m_control_empty;
    std::swap(m_localqueue_owned, m_local_own_empty);
    std::swap(m_localqueue_delegates, m_local_delegate_empty);
    std::swap(m_local_controller_queue, m_control_empty);
  }

  void print_size(){
    std::cout<<m_localqueue_owned.size()<<","<<m_localqueue_delegates.size()<<","<<m_local_controller_queue.size()<<std::endl;
  }
