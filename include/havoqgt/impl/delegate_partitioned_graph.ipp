...
void delegate_partitioned_graph<Allocator>::initialize_high_meta_data(
  ...
  for (int i = 0; i < processes_per_node; i++) {
    if (i == m_mpi_rank % processes_per_node) {
      ...
      // This is initlized during the paritioning of the low edges and then
      // adjusted in initialize_delegate_target
...
template <typename Allocator>
inline uint64_t delegate_partitioned_graph<Allocator>::outgoing_degree(
    delegate_partitioned_graph<Allocator>::vertex_locator locator) const {
  return degree(locator);
}

template <typename Allocator>
inline uint64_t delegate_partitioned_graph<Allocator>::incoming_degree(
    delegate_partitioned_graph<Allocator>::vertex_locator locator) const {
  if (locator.is_delegate()) {
    uint64_t label    = m_delegate_label[locator.local_id()];
    uint64_t local_id = label / uint64_t(m_mpi_size);
    return m_local_incoming_count[local_id];
  }
  return m_local_incoming_count[locator.local_id()];
}
