...
#define TIME_LIMIT_PC 30 // unit:s
timer_t id_pc;
void set_stop_flag_pc(union sigval val)
{
  // printf("%d\n",val.sival_int);
  // printf("%p\n",val.sival_ptr);
...
void register_timer_pc(bool &stop_flag)
{
  ...
  ent.sigev_notify_function = set_stop_flag_pc;
  ent.sigev_value.sival_ptr = &stop_flag;
  // printf("create timer\n");
  timer_create(CLOCK_MONOTONIC, &ent, &id_pc);

  /* start a timer */
  value.it_value.tv_sec = TIME_LIMIT_PC;
  value.it_value.tv_nsec = 0;
  value.it_interval.tv_sec = 0;
  value.it_interval.tv_nsec = 0;
  // printf("start timer\n");
  timer_settime(id_pc, 0, &value, NULL);
}
...
namespace prunejuice
{
  ...
  public:
    ...
    tppm_visitor(vertex_locator _vertex) : vertex(_vertex),
                                           itr_count(0),
                                           do_pass_token(false),
                                           is_init_step(true),
                                           ack_success(false),
    ...
    tppm_visitor(vertex_locator _vertex,
                 ...
                 bool _ack_success = false) : vertex(_vertex),
                                              parent(_parent),
            ...
            vertex_pattern_index == pattern_indices[next_pattern_index])
        {
              ...
              parent_pattern_index == pattern_indices[next_pattern_index - 1])
          {
            ...
            if (do_pass_token && (max_itr_count > itr_count))
            {
              ...
              if (!(vertex.is_delegate() && (g->master(vertex) != mpi_rank)))
              {
                if (std::get<18>(alg_data) && (vertex_pattern_index == std::get<20>(alg_data)))
                { // mark join vertex
                ...
                else if (std::get<19>(alg_data) && (vertex_pattern_index == std::get<20>(alg_data)))
                { // ignore join vertex
    ...
    bool visit(Graph &g, VisitorQueueHandle vis_queue, AlgData &alg_data) const
    {
      ...
      */
      ...
      if (!do_pass_token && is_init_step && itr_count == 0)
      {
        ...
        // if (!(find_vertex->second.vertex_pattern_index == pattern_indices[0] && vertex_data == pattern[0])) {
        if (std::get<16>(alg_data))
        { // pattern_selected_vertices // unique vertex metadata
          if ((vertex_data != pattern[0]) && (vertex_data != pattern[pattern.size() - 1]))
          {
            return false;
          }
        ...
        if (!pattern_valid_cycle && !std::get<16>(alg_data))
        { // path checking // not pattern_selected_vertices
          if (!(vertex_template_vertices.test(pattern_indices[0]) && vertex_template_vertices.test(pattern_indices[pattern_indices.size() - 1])))
          {
            return false;
          }
      ...
      { // else if
        ...
        else if (max_itr_count == itr_count)
        {
          ...
          // path checking
          std::cout<<"expect:"<<expect_target_vertex<<":"<<match_found<<std::endl;
          ...
          else if (g.locator_to_label(vertex) == g.locator_to_label(target_vertex) && match_found && expect_target_vertex)
          {
  ...
  { // TODO: bool& pattern_found does not work, why?
    ...
    auto alg_data = std::forward_as_tuple(vertex_metadata, pattern, pattern_indices, vertex_rank,
                                          ...
                                          pattern_mark_join_vertex, pattern_ignore_join_vertex, pattern_join_vertex, stop);
