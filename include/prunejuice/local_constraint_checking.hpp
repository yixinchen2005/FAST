...
public:
  ...
  {
  public:
    vertex_state_generic() : vertex_pattern_index(0),
                             is_active(false) {}
...
#ifdef ENABLE_MY_LCC
  template <typename Graph, typename Vertex, typename VertexData, typename BitSet, typename NLFMap>
#else
  ...
  public:
    ...
    lppm_visitor() : msg_type(0) {}

    lppm_visitor(vertex_locator _vertex, uint8_t _msg_type = 0) : vertex(_vertex),
                                                                  msg_type(_msg_type) {}

    lppm_visitor(vertex_locator _vertex, vertex_locator _parent,
                 size_t _parent_pattern_index, uint8_t _msg_type) : vertex(_vertex),
                                                                    parent(_parent),
  ...
  lppm_visitor(vertex_locator _vertex, ByteVector _parent_template_vertices, 
    ...
    lppm_visitor(vertex_locator _vertex, vertex_locator _parent,
                 BitSet _parent_template_vertices, uint8_t _msg_type) : vertex(_vertex),
                                                                        parent(_parent),
      ...
      if (vertex.is_delegate() && g->master(vertex) != mpi_rank && msg_type == 1)
      {
        ...
        if (std::get<8>(alg_data) == 0 && std::get<9>(alg_data))
        {
               ...
               vertex_pattern_index++)
          {
            ...
            {
              assert(vertex_pattern_index < 32); // Test
        ...
        { // if // redundent code is to avoid memory copy
...
//              std::cerr << "Error: did not find the expected item in the map." << std::endl; // TODO: is this correct?
      ...
      if (std::get<8>(alg_data) == 0 && std::get<9>(alg_data))
      {
        ...
        if (msg_type == 1)
        {
          ...
          if (vertex_template_vertices.none())
          {
            std::cerr << "Error: no bit is set.1" << std::endl;
            return false;
          }
      ...
      if (std::get<8>(alg_data) > 0 || !std::get<9>(alg_data))
      {
        ...
        else
        {
          ...
          if (msg_type == 1)
          {
            ...
            {
              std::cerr << "Error: no bit is set.2" << std::endl;
              return false;
            }
            ...
            if (vertex_template_vertices.none())
            {
              std::cerr << "Error: no bit is set.3" << std::endl;
              return false;
            }
    ...
    bool visit(Graph &g, VisitorQueueHandle vis_queue, AlgData &alg_data) const
    {
      ...
      { // if // redundent code is to avoid memory copy // TODO: use vertex_template_vertices pointer?
                                      ...
                                      TemplateVertex &template_vertices, VertexUint8MapCollection &vertex_active_edges_map)
  {
           ...
           vertex_pattern_index++)
      {
        if (v.second.template_vertices.test(vertex_pattern_index))
        {
          ...
          for (auto e = pattern_graph.vertices[vertex_pattern_index];
               e < pattern_graph.vertices[vertex_pattern_index + 1]; e++)
          {
            assert(pattern_graph.edges[e] < 32); // Test
    ...
    { // TODO: replace this with C++ map erase
  ...
  template <typename Vertex, typename VertexData, typename TGraph,
            ...
            typename PatternGraph, typename NLFMap>
#else
  template <typename Vertex, typename VertexData, typename TGraph,
            typename VertexMetaData, typename VertexStateMapGeneric, typename VertexActive,
            typename VertexUint8MapCollection, typename BitSet, typename TemplateVertex,
                                              ...
                                              std::ofstream &message_count_result_file)
  {
    ...
    // dummy //
#ifdef ENABLE_MY_LCC
    typedef lppm_visitor<TGraph, Vertex, VertexData, BitSet, NLFMap> visitor_type;
#else
